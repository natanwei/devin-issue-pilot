  ...
  return [
    ...
    // Issue #21 — scoped + yellow (Needs Input)
    {
      ...
      messages: [
        {
          role: "devin",
          text: "I've analyzed the migration requirements. I have two questions before I can confidently plan the implementation:\n\n1. Should the migration preserve SQLite as a fallback for local development, or fully replace it?\n2. What are the schema constraints for foreign keys — should they use CASCADE or RESTRICT on delete?",
          timestamp: "2026-02-03T08:55:00Z",
          source: "app",
        },
        {
          role: "user",
          text: "Keep SQLite for local dev with an env flag. Use CASCADE for all foreign keys.",
      ...
      steps: [],
      scoping_session: {
        session_id: "demo-scope-21",
        session_url: "https://app.devin.ai/sessions/demo-scope-21",
    ...
    // Issue #9 — scoped + yellow (Needs Input)
    {
      ...
      messages: [
        {
          role: "devin",
          text: "I need a couple of clarifications before I can scope the rate limiting middleware:\n\n1. What are the desired rate limits (requests per minute)?\n2. Should rate limiting use Redis or in-memory storage?",
      ...
      steps: [],
      scoping_session: {
        session_id: "demo-scope-9",
        session_url: "https://app.devin.ai/sessions/demo-scope-9",
    ...
    // Issue #5 — scoped + red (Unclear)
    {
      ...
      messages: [
        {
          role: "devin",
          text: "The issue description is too vague for me to scope confidently. I need more details:\n\n1. What specific behavior is expected after the migration?\n2. Which tables need to be migrated and what are the schema constraints?",
      ...
      steps: [],
      scoping_session: {
        session_id: "demo-scope-5",
        session_url: "https://app.devin.ai/sessions/demo-scope-5",
        started_at: "2026-02-01T13:30:00Z",
      },
      fix_session: null,
      last_devin_comment_id: null,
      last_devin_comment_at: null,
    ...
    // Issue #12 — scoped + red (Unclear)
    {
      ...
      messages: [
        {
          role: "devin",
          text: "This issue is too broad for a single fix. I have two critical questions:\n\n1. Should services use Result types, throw exceptions, or return error objects?\n2. Which services are highest priority to fix first?",
          timestamp: "2026-02-01T09:50:00Z",
          source: "app",
        },
        {
          role: "user",
          text: "Use Result types everywhere. Start with the auth and payment services.",
          timestamp: "2026-02-01T09:55:00Z",
          source: "app",
        },
        {
          role: "devin",
          text: "Thanks for the clarification. However, I still can't confidently scope this — the auth service alone touches 8 files and the payment service has no test coverage. I'd recommend splitting this into two separate issues.",
      ...
      steps: [],
      scoping_session: {
        session_id: "demo-scope-12",
        session_url: "https://app.devin.ai/sessions/demo-scope-12",
    ...
    // Issue #3 — blocked
    {
      ...
      messages: [
        {
          role: "devin",
          text: "I've started implementing the OAuth2 flow and completed the error handler update. However, I've hit a blocker:\n\nThe database.ts file uses a custom connection pool wrapper (DBPool) that doesn't expose a standard health check method. The try-catch approach won't work without modifying the pool interface.",
          timestamp: "2026-02-07T08:45:00Z",
          source: "app",
        },
        {
          role: "user",
          text: "Can you add a .isHealthy() method to the DBPool class? It's in src/lib/pool.ts.",
          timestamp: "2026-02-07T08:50:00Z",
          source: "app",
        },
        {
          role: "devin",
          text: "I found the file but it's not in the original scope. I'd need to modify src/lib/pool.ts — should I proceed with that change?",
